...
public class VideoListAdapter extends ArrayAdapter<MediaItem> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mm.hasImage()) {
            aq.id(holder.imgView).width(110).image(mm.getImage(0),
                    true, true, 0, R.drawable.default_video, null, 0, mAspectRatio);
        }
        aq.id(holder.titleView).text(mm.getTitle());
        aq.id(holder.descrView).text(mm.getSubTitle());
